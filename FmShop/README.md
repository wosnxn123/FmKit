...
> 服务器商店 —— 自带整数分经济，菜单左键买、右键卖，价格表素行 `buy = 2 × sell`、附魔书行 `buy = 6 × sell` 断掉刷钱回路。

FmShop 是 Folia / Paper 服务端的服务器商店插件，自带经济，不依赖 Vault。金额全程以 `long` 分存储（1 金币 = 100 分），除配置读取与展示外不出现浮点，因此不存在累积误差。玩家用 `/fmshop` 打开菜单买卖，也可以用指令直接成交；两条路径共用同一套交易引擎与同一份文案，读起来完全一致。默认价格表覆盖服务端全部可交易物品 —— 1502 件商品、15 个分类（1374 件手写行 + 128 行曲线展开的附魔书；素行买价等于卖价两倍，附魔书行买价为卖价六倍）。非天然物品的买价不是手填的，而是按服务端真实配方递推：`buy(产物) = 1.25 × 最便宜合法配料买价之和 ÷ 产量`，于是「买材料 → 合成 → 卖成品」在任意深度的合成链上恒定亏损（0.5 × 1.25 = 0.625 < 1）；天然产出（矿石、花草、怪物掉落、结构战利品）逐条手工定锚。所有 `*_ORE` 与硫磺尖刺 `buy: 0` 只回收不出售，时运镐无法把 1 份矿石变成 4 份掉落；沙砾→燧石、萤石→萤石粉、紫水晶簇→碎片这类破方块掉落逐个校验过「方块买价 ≥ 时运 III 上限掉落的卖价」。`/fsa doctor` 再拿服务端真实配方表复查一遍。设计细节、完整配置键表与线程模型见 [DESIGN.md](DESIGN.md)。
...
- **图形化商店**：买与卖挂在同一个商品图标的左、右键上，玩家不用找模式开关；Shift+右键立即卖出全部持有；确认界面按 ±1 / ±16 / ±64 调量，预览写明单价、小计、手续费、应付或可得，以及数量上限及其原因；一键回收在成交前列出物品种类、合计、手续费与预计可得，施加与交易引擎完全相同的夹取，回收按商品行探针精确匹配 —— 素物品要求未附魔、未改名、未损伤，附魔书只有与某一行完全一致（同种附魔同等级、无额外附魔、未改名）才收，多带一条附魔的书报「不回收」；菜单按 `gui.auto-refresh-seconds` 周期重绘，挂着菜单跨过一次价格波动时按新价成交，不兑现旧数字；菜单布局：分类首页 54 格、三排 21 个分类位（当前用去 15 个，底栏 47 我的余额 / 49 一键回收 / 51 关闭），分类页每页 45 件（45 上一页 / 53 下一页），确认页 27 格；
- **反套利价格表**：素行 `buy = 2 × sell`，附魔书行 `buy = 6 × sell`；买卖共用同一个行情倍率，任何倍率下差价比例不变，动态定价永远打不开刷钱回路；`/fsa doctor` 用服务端真实配方表复查。配料的买价已是其卖价的两倍，任何配方产物都不可能比配料更值钱；方块（9 合 1）按 9× 精确定价，熔炼保证产物卖价不高于原料买价；复查覆盖有形 / 无形 / 熔炼 / 切石四类配方，存在正利润的路径记为错误；
- **附魔书按曲线定价**：附魔 分类 128 行（26.2 原版 43 种附魔 × 各自等级）不逐条手写，由 `prices.yml` 的 `enchanted-books:` 块按曲线展开；每种书终身限售 1 本、卖出后才永久解锁购买 —— 详见下文「附魔书」一节；
- **可选动态行情**：默认关闭；开启后买入抬价、卖出压价，倍率夹在 `floor-percent` 与 `ceil-percent` 之间，按 `recover-bp-per-hour` 在读取时惰性回归基准，没有定时任务；
- **每日限额与日切**：每件商品可单独设 `daily-buy` / `daily-sell`，额度是**每名玩家各自一份**（不是全服共享池，也没有商店库存），计数在 `limits.reset-hour`（默认 4 点，服务器本地时间）翻页；被限额夹住的订单按实际可成交量成交，并如实上报剩余量与重置倒计时，而不是整单拒绝；
...
附魔 分类有 128 行商品 —— 26.2 原版 43 种附魔各按等级展开（锋利 I…V 是 5 行），分类页每页 45 件，共 3 页。这些行不是手写的，而是 `prices.yml` 末尾 `enchanted-books:` 配置块按曲线展开的：

```
sell = base-sell × 等级^level-exponent × rarity^rarity-exponent × 诅咒系数
buy  = sell × buy-multiplier（出厂 6 倍）
```

出厂参数：`base-sell 24`、`level-exponent 1.7`、`rarity-exponent 1`、`buy-multiplier 6`、`curse-multiplier 0.25`；`rarity` 默认取原版 `Enchantment#getAnvilCost()`，`overrides` 只调个别附魔的 rarity（如经验修补 25、荆棘 0.75）。例如锋利 V：买入 2,221.32，卖出 370.22（卖出再扣 5% 手续费，净得 351.71）。

玩法规则（出厂默认）：

- **终身限售 1 本**（`lifetime-sell: 1`）：每名玩家每种书终身只能卖给商店 1 本，永不重置；
- **卖后解锁购买**（`unlock-by-sell: true`）：没卖出过的书买不了，提示「尚未解锁 —— 先向商店卖出 1 个，即可永久解锁购买」；
- **精确匹配才回收**：只有与某一行完全一致的书（同种附魔同等级、无额外附魔、未改名）可卖；一本锋利 V + 耐久 III 的书报「不回收」，不会落进任何其它行。

指令里按行 id 引用书：`/fmshop price sharpness/5` 即锋利 V（完整 id 形如 `ENCHANTED_BOOK/SHARPNESS/5`）。`/fsa price` 对这些行拒改，提示去 `prices.yml` 的 `enchanted-books` 区段改；`/fsa resetunlock <玩家>` 清除该玩家的终身计数，附魔书随之重新上锁。
...
玩家指令 `/fmshop`（别名 `fsh`，权限 `fmshop.use`）：
...
物品名支持唯一前缀匹配，`/fmshop price sulfur_sp` 即可命中 `SULFUR_SPIKE`；附魔书按行 id 匹配，`/fmshop price sharpness/5` 命中锋利 V（单写 `sharpness` 同时命中 5 个等级，因不唯一而不解析）。
...
| `/fsa price <物品> <买价> <卖价>` | 改价：写回 `prices.yml` 后自动重载；卖价不低于买价直接拒绝；附魔书行由 `enchanted-books` 块生成，此指令拒改 |
| `/fsa market <物品> [reset]` | 查看行情倍率、现价、基准价与累计买卖量；`reset` 复位该商品行情 |
| `/fsa resetlimit <玩家\|*>` | 重置今日限额；`*` 作用于全部在线玩家 |
| `/fsa resetunlock <玩家>` | 清除该玩家的终身限售计数（soldEver），附魔书重新上锁 |
| `/fsa reload` | 重读配置与价格表 |
| `/fsa status` | 商品数、账户数、费率与去向、税池、行情波动数、打开的界面数、日志文件数、限额重置时刻与严格模式 |
...
逐条指令的完整说明（参数、补全、错误文案）见 [指令手册](../docs/指令手册.md)。
...
商品价格、分类与动态行情参数在 `plugins/FmShop/prices.yml`；逐键说明见 [配置指南](../docs/配置指南.md)，完整键表、夹取规则与取值理由见 [DESIGN.md](DESIGN.md)。
...
`build.ps1` 的 classpath 默认取 `%USERPROFILE%\.m2\repository`，可用环境变量 `M2_REPO` 覆盖。实服端到端回归见 [bot/fmshop_test.js](../bot/fmshop_test.js)（mineflayer 操作菜单 + RCON 下管理指令，断言余额、限额、账本文件与配置重载后的持久性），构建细节与回归断言清单见 [构建与测试](../docs/构建与测试.md)。
...
└── src/main/
    ...
    │   ├── store/
    │   │   ├── ItemKey.java        # 行身份：材料 + 可选存储附魔，id 为查找 / 存档键
    │   │   ├── EnchantedBooks.java # enchanted-books: 按曲线展开为 128 行
    ...
    │   │   └── PlayerData.java     # 余额、日号、今日买卖与终身计数
    │   ├── tx/
    ...
    └── resources/
        ...
        └── prices.yml              # 1374 件手写商品 + 128 行曲线附魔书 / 15 个分类
```
