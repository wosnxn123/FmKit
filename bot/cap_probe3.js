...
async function scenario() {
  ...
  bot._client.on('window_items', p => {
    ...
    if (p.windowId === (bot.currentWindow ? bot.currentWindow.id : -1) && p.items && p.items[0]) {
      const raw = p.items[0]
      console.log('window_items slot0 FULL:', JSON.stringify(raw), 'keys:', Object.keys(raw||{}))
    }
  })
  bot._client.on('set_slot', p => {
    if (p.windowId === (bot.currentWindow ? bot.currentWindow.id : -1) && p.slot === 0) {
      console.log('set_slot slot0 FULL:', JSON.stringify(p.item), 'keys:', Object.keys(p.item||{}))
    }
  })
  ...
  await sleep(1500)
  console.log('parsed slot0:', win.slots[0] ? `${win.slots[0].name}x${win.slots[0].count}` : 'empty')
